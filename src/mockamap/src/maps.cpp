 ...
 */
{
  ...
  // info.cloud->points.push_back(pcl::PointXYZ(0,0,0));
  info.cloud->width    = info.cloud->points.size();
  info.cloud->height   = 1;
  info.cloud->is_dense = true;
...
Maps::MapGenerate()
{
  ...
  for (int i = 0; i < obscenter_x.size(); i++)
  {
    double x, y;
    x = obscenter_x[i];
    y = obscenter_y[i];
      ...
      for (int s = sl; s < sh; s++)
      {
        for (int t = 0; t < heiNum; t++)
        {
          if ((r - rl) * (r - rh + 1) * (s - sl) * (s - sh + 1) * t *
              ...
              0)
          {
            pt_manual.x = x + r * _resolution;
            pt_manual.y = y + s * _resolution;
            pt_manual.z = t * _resolution;
  ...
  //     pt_manual.x = x;
  //     pt_manual.y = _y_l;
  //     pt_manual.z = _h_h + _resolution; // 顶部边界高于最高障碍
  //     info.cloud->points.push_back(pt_manual);
  // }
  // for (double y = _y_l; y <= _y_h; y += _resolution) {
  //     pt_manual.y = y;
  //     pt_manual.x = _x_l;
  //     pt_manual.z = _h_h + _resolution; // 同上
  //     info.cloud->points.push_back(pt_manual);
  //     pt_manual.x = _x_h;
  //     info.cloud->points.push_back(pt_manual);
  // }
